    ...
    public class Quantity<U>
    {
        ...
        private double ConvertToBase(double val, U u)
        {
            if (u is LengthUnit lu)
                return lu.ConvertToBaseUnit(val);

            if (u is WeightUnit wu)
                return wu.ConvertToBaseUnit(val);

            if (u is VolumeUnit vu)
                return vu.ConvertToBaseUnit(val);
        ...
        {
            if (u is LengthUnit lu)
                return lu.ConvertFromBaseUnit(baseVal);

            if (u is WeightUnit wu)
                return wu.ConvertFromBaseUnit(baseVal);

            if (u is VolumeUnit vu)
                return vu.ConvertFromBaseUnit(baseVal);
